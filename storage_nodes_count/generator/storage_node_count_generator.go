...
import (
	"fmt"
	storagenodescount "github.com/libopenstorage/cloudops/storage_nodes_count"
	"math"
)
...
func main() {
	...
	if err := storagenodescount.NewStorageNodesMatrixParser().MarshalToYaml(&matrix, storagenodescount.SncYamlPath); err != nil {
		fmt.Println("Failed to generate storage nodes count decision matrix yaml: ", err)
		return
	}
	fmt.Println("Generated storage nodes decision matrix yaml at ", storagenodescount.SncYamlPath)
}
